import '../css/styles.css';  // Make sure Webpack processes the CSS file
...
function game() {
    ...
    function resetGame() {
        ...
        explosions = [];
        player.deg = 0;
        finalScoreText.textContent = `Total Destroyed: 0`;
    }
    ...
    function showGameOver() {
        playing = false;
        gameOver = true;
        finalScoreText.textContent = `Total Destroyed: ${destroyed}`;
    ...
    function action(e) {
        ...
        if (playing) {
            ...
            if (gameOver) {
                var dist = Math.sqrt(((e.offsetX - cW / 2) * (e.offsetX - cW / 2)) + ((e.offsetY - (cH / 2 + 45 + 22)) * (e.offsetY - (cH / 2 + 45 + 22))));
                if (dist < 27) {
                    if (e.type == 'click') {
                        gameOver = false;
                        count = 0;
